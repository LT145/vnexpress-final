...
const CommentsPage = () => {
  ...
  const groupedComments: Record<string, Comment[]> = comments.reduce((acc, comment) => {
  ...
  const sortedGroupedComments = Object.fromEntries(
    Object.entries(groupedComments).map(([postTitle, comments]) => [
      postTitle,
      [...comments].sort((a, b) => {
        if ((a.status === 'APPROVED' || a.status === 'REJECTED') && (b.status !== 'APPROVED' && b.status !== 'REJECTED')) return 1;
        if ((b.status === 'APPROVED' || b.status === 'REJECTED') && (a.status !== 'APPROVED' && a.status !== 'REJECTED')) return -1;
  ...
  const sortedPosts = Object.entries(sortedGroupedComments).sort(([, aComments], [, bComments]) => {
    const aOldest = aComments.reduce((oldest, current) => oldest.createdAt < current.createdAt ? oldest : current);
    const bOldest = bComments.reduce((oldest, current) => oldest.createdAt < current.createdAt ? oldest : current);
      ...
      setComments(prev =>
        prev.map(comment =>
          comment.id === result.id
            ? { ...comment, ...result } // giữ lại postTitle từ comment cũ nếu backend không trả về
            : comment
        )
      );
    } catch (error) {
        ...
        {sortedPosts.map(([postTitle, comments]) => (
          <div key={postTitle} className="mt-2 border rounded-lg shadow-sm">
            <div className="flex items-center justify-between p-4 bg-gray-50 border-b rounded-t-lg">
              ...
              <Button variant="outline" size="sm" onClick={() => togglePost(postTitle)}>
